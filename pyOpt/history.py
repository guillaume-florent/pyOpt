...
# To Do:
#     - shelving of optimizer


class History(object):
    """Abstract Class for Optimizer History Object"""
    def __init__(self, filename, mode, optimizer=None, opt_prob=None):
        ...
        **Keyword arguments:**
        ...
        if self.mode == 'w':
            ...
            if not os.path.isfile(bin_name):
                raise NameError('Error: filename %s.bin does not exist' %
                                filename)
            if not os.path.isfile(cue_name):
                raise NameError('Error: filename %s.cue does not exist' %
                                filename)
            # if not os.path.isfile(opt_name):
            #     raise NameError('Error: filename %s.opt does not exist' % \
            #                     filename)
...
#               print 'Warning Optimizer Instance and stored Optimizer '
#                      'do not match -- hot start aborted'
#               self.cue_file.close()
#               self.opt_file.close()
#               self.s_count = 0
#               return
            ...
            for line in lines[1:]:
                if len(line) < 3:
                    ...
                    if tline[2] in self.cues:
                        self.cues[tline[2]].append([int(tline[0]),
                                                    int(tline[1])])
                    else:
    ...
    def read(self, index=None, ident=None):
        """Read Data from Optimizer History Files

        **Keyword arguments:**

        - index -> LIST,SCALAR: Index (list), [0,-1] for all,
                   [] internal count, -1 for last, *Default* = []
        ...
        for id_ in ident:
            bdata[id_] = []
            if id_ in self.cues.keys():
                if isinstance(index, int):
                    if index == -1:
                        index = len(self.cues[id_])-1
                    ...
                    if bool(index) is False:  # empty list evaluates to False
                        ...
                        index = [0, len(self.cues[id_])]
                else:
                    raise ValueError('Index type not understood - '
                                     'must be either int or list')
            else:
                hist_end = True
                return bdata, hist_end
            for i in range(index[0], index[1]):
                if i >= len(self.cues[id_]):
                    hist_end = True
                    return bdata, hist_end
                tvals = ARRAY.array('d')
                self.bin_file.seek(self.cues[id_][i][0]*8, 0)
                tvals.fromfile(self.bin_file, self.cues[id_][i][1])
                bdata[id_].append(numpy.array(tvals))
