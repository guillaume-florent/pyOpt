...
class Variable(object):
    r"""Optimization Variable Class"""

    def __init__(self, name, type_='c', value=0.0, **kwargs):
        r"""Variable Class Initialization
        ...
        **Keyword arguments:**
        ...
        - type_ -> STR: Variable Type ('c'-continuous,
                                      'i'-integer,
        ...
        if type_[0].lower() == 'c':
            ...
            for key in kwargs.keys():
                if key == 'choices':
                    ...
                    raise IOError('A discrete variable requires to input '
                                  'an array of choices')
            try:
                self.value = self.choices[int(value)]
            except:
                raise IOError('A discrete variable requires the value input to '
                              'be a integer pointer value of the choices array')

            self.lower = int(0)
            self.upper = int(len(self.choices))
        else:
            raise IOError('Variable type not understood -- '
                          'use either c(ontinuous), i(nteger) or d(iscrete)')
