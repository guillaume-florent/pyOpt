...
"""alpso_spm - Parallel Python Version of the Augmented Lagrangian
...
-------
    v. 1.0	- Initial Class Creation (PJ, 2008)
            ...
            - Removed lambda convergence criteria (PJ, 2009)
            - Added Multiple x0s Input Functionality (PJ, 2010)

Note: This implementation does not allow multiple successive call
to the optimizer.
...
def alpso(dimensions,
    ...
    if (fileout == 2) or (fileout == 3):
        ...
        stext += '-' * 97 + '\n'
        stext += "    Major   Minor   nFCon   Violation(L2)     Objective   " \
                 "Lagrangian   Rel Lagrangian   Global Dist\n"
        stext += '-' * 97 + '\n'
        sfile.write(stext)
        sfile.flush()
    ...
    while (k_out < maxOutIter) and (stop_main_flag == 0):
        ...
        # Inner optimization loop - core ALPSO algorithm
        # applied to the lagrangian function
        k_inn = 0
        stop_inner = 0
        while (k_inn < maxInnIter) and (stop_inner == 0):
            ...
            # calculating new search radius for the
            # best particle ("Guaranteed Convergence" method)
            ...
            w = w2 + ((w2 - w1) / global_distance_reference) * global_distance[
                1]
            if w > w1:
                w = w1
            elif w < w2:
            ...
            if prtInnIter != 0 and numpy.mod(k_inn, prtInnIter) == 0:
                # output to screen
                print('%d Inner Iteration of %d Outer Iteration' % (k_inn,
                                                                    k_out))
        ...
        if myrank == 0:
            ...
            if stopCriteria == 1:
                ...
                # setting up the stopping criteria based
                # on distance and tolerance
                ...
                # relative extent of the swarm
                global_distance[0] = global_dist / swarmsize
            ...
            if stop_main_flag == 0:
                ...
                if constraints > 0:
                    ...
                    for l in range(neqcons):
                        if (abs(swarm_g[l]) > abs(swarm_g_old[l]) and abs(
                                swarm_g[l]) > etol):
                            rp[l] *= 2.0
