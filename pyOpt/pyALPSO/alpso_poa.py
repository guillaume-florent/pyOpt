        ...
        if x0 != []:
            if len(x0.shape) == 1:
                ...
                if x0.shape[0] > swarmsize:
                    if myrank == 0:
                        print('Warning: %d initial positions specified for '
                              '%d particles, last %d positions '
                              'ignored' % (x0.shape[0],
                                           swarmsize,
                                           x0.shape[0] - swarmsize))
    ...
    while (k_out < maxOutIter) and (stop_main_flag == 0):
        ...
        while (k_inn < maxInnIter) and (stop_inner == 0):
            ...
            if prtInnIter != 0 and numpy.mod(k_inn, prtInnIter) == 0:
                # output to screen
                print('Outer Iteration: %d \t[%d. Inner Iteration]' % (k_out,
                                                                       k_inn))
        ...
        if myrank == 0:
            ...
            if stop_main_flag == 0:
                ...
                if constraints > 0:
                    ...
                    for l in range(constraints):
                        swarm_L_old += lambda_val[l] * tau_old[l] + \
                                       rp[l] * tau_old[l] ** 2
