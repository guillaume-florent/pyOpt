    ...
    if x0 != []:
        if len(x0.shape) == 1:
            ...
            if x0.shape[0] > swarmsize:
                print('Warning: %d initial positions specified for %d '
                      'particles, last %d positions ignored' %
    ...
    while (k_out < maxOutIter) and (stop_main_flag == 0):
        ...
        while (k_inn < maxInnIter) and (stop_inner == 0):
            ...
            for i in range(swarmsize):
                ...
                if (nhm == 'dlring') or (nhm == 'slring') or (nhm == 'wheel') \
                        or (nhm == 'spatial') or (nhm == 'sfrac'):
                    lbest_x = nhbest_x[i, :]
                else:
                    lbest_x = swarm_x[:]
            ...
            # If there is no new better solution for gbest
            # keep the old best position
        ...
        # Test Convergence
        if (
                stop_constraints_flag == 1 and stop_criteria_flag == 1):  # and stop_lambda_flag == 1
            stop_main_flag = 1
        else:
            stop_main_flag = 0
