...
class ALPSO(Optimizer):
    ...
    def __init__(self, pll_type=None, *args, **kwargs):
        ...
        **Keyword arguments:**
        ...
        - pll_type -> STR: ALPSO Parallel Implementation   
                        None,
                        SPM- Static,
                        DPM- Dynamic,
                        POA-Parallel Analysis),
                        *Default* = None
        ...
        def_opts = {
            ...
            'stopCriteria': [int, 1],
            # Consecutively Number of Iterations for which the
            # Stopping Criteria must be Satisfied
            'stopIters': [int, 5],
            # Absolute Tolerance for Equality constraints
            'etol': [float, 1e-3],
            # Absolute Tolerance for Inequality constraints
            'itol': [float, 1e-3],
            # Absolute Tolerance for Lagrange Multipliers
            # 'ltol':[float,1e-2],
            # Relative Tolerance for Lagrange Multipliers
            'rtol': [float, 1e-2],
            # Absolute Tolerance for Lagrange Function
            'atol': [float, 1e-2],
            # Relative Tolerance in Distance of All Particles
            # to Terminate (GCPSO)
            'dtol': [float, 1e-1],
            ...
            'ns': [int, 15],
            # Number of Consecutive Failures in Finding New Best Position of
            # Best Particle Before Search Radius will be Increased (GCPSO)
            'nf': [int, 5],
            # Time step
            'dt': [float, 1.0],
            ...
            'HoodSize': [int, 40],
            # Neighbourhood Model (dl/slring - Double/Single Link Ring, wheel -
            # Wheel, Spatial - based on spatial distance,
            # sfrac - Spatial Fraction)
            'HoodModel': [str, 'gbest'],
            # Selfless Neighbourhood Model (0 - Include Particle i in NH i,
            #                               1 - Don't Include Particle i)
            'HoodSelf': [int, 1],
            # Design Variables Scaling Flag (0 - no scaling,
            #                                1 - scaling between [-1,1])
            'Scaling': [int, 1],
        }
        informs = {}
    ...
    def __solve__(self,
        ...
        **Keyword arguments:**
        ...
        if (self.options['HoodModel'][1].lower() in ['gbest', 'dlring',
                                                     'slring', 'wheel',
                                                     'spatial', 'sfrac']):
        ...
        if 0 <= self.options['stopCriteria'][1] <= 1:
        ...
        # Run ALPSO
        t0 = time.time()
        opt_x, opt_f, opt_g, opt_lambda, nfevals, rseed = \
            self.alpso.alpso(n, m, me, type, xs, xl, xu, nob, nhn, nhm, imax,
                             cmax, cmin, stop, nstop, etol, itol, rtol, atol,
                             dtol,  oout, iout, rinit, vinit, vmax, c1, c2,
                             w1, w2, ns, nf, vcrazy, fileout, filename,
                             log_file, hos_file, seed, scale, nhs, objconfunc)
        ...
        if store_sol:
            ...
            opt_problem.addSol(self.__class__.__name__, sol_name, objfunc,
                               ...
                               Seed=rseed, myrank=self.myrank,
                               arguments=args, **kwargs)

        return opt_f, opt_x, {'opt_g': opt_g,
                              'fevals': sol_evals,
                              'time': sol_time}
