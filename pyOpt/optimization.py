...
from pyOpt.variable import Variable
from pyOpt.objective import Objective
from pyOpt.constraint import Constraint
...
class Optimization(object):
    ...
    def addVar(self, *args, **kwargs):
        ...
        """
        id_ = self.firstavailableindex(self._variables)
        self.setVar(id_, *args, **kwargs)

        # tmp_group = {}
        # tmp_group[self._variables[id_].name] = id_
        tmp_group = {self._variables[id_].name: id_}
        self._vargroups[self.firstavailableindex(self._vargroups)] = \
            {'name': self._variables[id_].name, 'ids': tmp_group}

    def addVarGroup(self, name, nvars, type_='c', value=0.0, **kwargs):
        ...
        **Keyword arguments:**
        ...
        - type -> STR: Variable type ('c'-continuous,
                                      'i'-integer,
                                      'd'-discrete),
                                      *Default* = 'c'
        ...
        if isinstance(value, list) or isinstance(value, numpy.ndarray):
            ...
            raise IOError('Variable type for value not understood - '
                          'use float, int or list\n')
        ...
        for key in kwargs.keys():
            if key == 'lower':
                if isinstance(kwargs['lower'], float):
                    ...
                    raise IOError('Variable type for lower bound not '
                                  'understood - use float, int or list\n')
            elif key == 'upper':
                if isinstance(kwargs['upper'], float):
                    ...
                    raise IOError('Variable type for upper bound not '
                                  'understood - use float, int or list\n')
            if key == 'choices':
        ...
        for var in range(nvars):
            ...
            self.setVar(id_,
                        ...
                        upper=upper[var],
                        choices=choices[var])
            tmp_group[tmp_name] = id_

        self._vargroups[self.firstavailableindex(self._vargroups)] = \
            {'name': name, 'ids': tmp_group}
        ...
        **Arguments:**
        ...
        if (len(args) > 0) and isinstance(args[0], Variable):
            ...
            try:
                ...
                raise ValueError("Input is not a Valid for a "
        ...
        **Arguments:**
        ...
        for j in range(ngroups):
            if self._vargroups[j]['name'] == name:
                keys = self._vargroups[j]['ids']
                for key in keys:
                    id_ = self._vargroups[j]['ids'][key]
                    del self._variables[id_]
                del self._vargroups[j]
    ...
    def addCon(self, *args, **kwargs):
        ...
        #
        self.setCon(self.firstavailableindex(self._constraints),
                    *args,
                    **kwargs)

    def addConGroup(self, name, ncons, type_='i', **kwargs):
...
#        self._pargroups[self.firstavailableindex(self._pargroups)] = \
#                             {'name':self._parameters[id].name,'ids':tmp_group}
#
#
...
#        return self._pargroups
    ...
    def firstavailableindex(self, set_):
    ...
    def write2file(self, outfile='', disp_sols=False, **kwargs):
        ...
        **Keyword arguments:**
        ...
        print('Data written to file %s' % outfile.name)
        outfile.close()
...
class Solution(Optimization):
    ...
    def __init__(self,
        ...
        **Keyword arguments:**
        ...
        - myrank -> INT: Process identification for MPI evaluations,
                    *Default* = 0
    ...
    def __str__(self):
        ...
        if self.display_opt:
            ...
            for key in opt_keys:
                ns = 25-len(key)
                text1 += '		' + key + ':' + \
                         str(self.options_set[key][1]).rjust(ns, '.') + '\n'
        ...
        for key in self.parameters.keys():
            if (isinstance(self.parameters[key], (dict, list, tuple))) and \
                    (len(self.parameters[key]) == 0):
                continue
            elif (isinstance(self.parameters[key], numpy.ndarray)) and \
                    (0 in (self.parameters[key]).shape):
                continue
            else:
                text1 += '    ' + key + ': ' + \
                         str(self.parameters[key]).rjust(9) + '\n'
