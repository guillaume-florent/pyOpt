...
class MIDACO(Optimizer):
    ...
    def __init__(self, pll_type=None, *args, **kwargs):
        ...
        **Keyword arguments:**
        ...
        def_opts = {
            # MIDACO Options
            # Accuracy for constraint violation (0 - default)
            'ACC': [float, 0],
            # Seed for random number generator  (e.g. ISEED = 0,1,2,3,...)
            'ISEED': [int, 0],
            # Objective Function Stopping Value (0 - disabled)
            'FSTOP': [int, 0],
            # Automatic stopping criteria (0 - disable,
            #                              1 to 500 - from local to global)
            'AUTOSTOP': [int, 0],
            ...
            'KERNEL': [int, 0],
            # Internal custom parameters (0 - Use internal default,
            #                             1 - IP problems,
            #                             2 - NLP problems,
            #                             3 - MINLP problems)
            'CHARACTER': [int, 0],
            ...
            'MAXTIME': [int, 86400],
            # Output Level (<0 - None, 0 - Screen, 1 - File(s))
            'IPRINT': [int, 1],
        ...
        **Keyword arguments:**
        ...
        if self.poa or self.spm:
            try:
                ...
                print('pyMIDACO: Parallel objective Function Analysis '
                      'requires mpi4py')
        ...
        # Run MIDACO
        t0 = time.time()
        midaco.midaco_wrap(ll, nn, ni, mm, me, xx, xl, xu, ff, gg, param,
                           maxeval, maxtime, ifail, neval, iprint, printeval,
                           iout1, iout2, ifile1, ifile2, lkey, liw, iw, lrw, rw,
                           objfun)
