...
class NLPQLP(Optimizer):
    ...
    def __init__(self, pll_type=None, *args, **kwargs):
        ...
        **Keyword arguments:**
        ...
        if pll_type is None:
            ...
            raise ValueError("pll_type must be either "
                             "None, 'SPM', 'DPM' or 'POA'")
    ...
    def __solve__(self, opt_problem={}, sens_type='FD', store_sol=True,
        ...
        **Keyword arguments:**
        ...
        lwa = numpy.array([23 * nmax + 4 * mmax + 3 * mmax + 150 +
                           3 * nmax * nmax / 2 + 10 * nmax
                           + nmax + mmax + 1], numpy.int)
        ...
        # Run NLPQLP
        t0 = time.time()
        xx, ff, gg, uu, nfun, ngrd = nlpqlp.nlpqlp_wrap(ll, mm, me, mmax, nn,
                                                        nmax, mnn2, xx, ff, gg,
                                                        df, dg, uu, xl, xu, cc,
                                                        dd, acc, accqp, stpmin,
                                                        maxfun, maxit, maxnm,
                                                        rhob, mode, ifail,
                                                        iprint, iout, ifile, wa,
                                                        lwa, kwa, lkwa, active,
                                                        lactiv, lql, nfun, ngrd,
                                                        nlfunc, nlgrad)
