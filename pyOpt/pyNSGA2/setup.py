...
from numpy.distutils.misc_util import appendpath, Configuration
        ...
        if ext == '.i':  # SWIG interface file
            ...
            if os.path.isfile(source):
                ...
                if typ is None:
                    ...
                    if typ != typ2:
                        log.warn('expected %r but source %r '
                                 'defines %r swig target'
                                 % (typ, source, typ2))
                        if typ2 == 'c++':
                            log.warn('resetting swig target to c++ '
                                     '(some targets may have .c extension)')
                            is_cpp = True
                            target_ext = '.cpp'
                        else:
