...
def alhso(dimensions,
    ...
    # if x0 != []:
    if x0 is True:  # non empty list evaluates to True
        if scale == 1:
    ...
    for i in range(memsize):
        ...
        if constraints > 0:
            ...
            for l in range(constraints):
                L_val += lambda_val[l] * tau_val[l] + rp_val[l] * tau_val[
                                                                      l] ** 2
    ...
    while (k_out < maxoutiter) and (stop_main_flag == 0):
        ...
        while k_inn < maxinniter:
            ...
            if constraints > 0:
                ...
                for l in range(constraints):
                    L_val += lambda_val[l] * tau_val[l] + \
                             rp_val[l] * tau_val[l] ** 2
        ...
        if best_f_val == [] and k_out == 1 and x0 == []:
            ...
            for i in range(memsize):
                ...
                if constraints > 0:
                    ...
                    for l in range(constraints):
                        L_val += lambda_val[l] * tau_val[l] + rp_val[l] * \
                                                              tau_val[l] ** 2
        ...
        if stop_main_flag == 0:
            if constraints > 0:
                ...
                for l in range(neqcons):
                    if (abs(best_g_val[l]) > abs(best_g_old[l]) and abs(
                            best_g_val[l]) > etol):
                        rp_val[l] *= 2.0
...
def chso(ND,
         ...
         rseed,
         objfunc):
    """CHSO function - Python Version of the Constrained 
    ...
    while status != 1:
        ...
        if sum(gvals) <= 0:
            ...
            if fval == hmin:
                ...
                opt_g = gvals
                if printout == 1:
                    # print('%f,%f,%f,%f' %(k,x,fval,
                    #                          numpy.var(numpy.corrcoef(HM).T)))
                    print('%i,%f' % (k, fval))
